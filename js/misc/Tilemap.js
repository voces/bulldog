...
/* globals TERRAIN, Tile, app, Unit, BinaryHeap, Destructible */
...
class Tilemap {
	...
	xWorldToTile( x ) {
		...
		return Math.floor( ( x + this.realWidth / 2 ) / TERRAIN.TILE_SIZE );

	}
	yWorldToTile( y ) {

		return this.height - Math.floor( ( y + this.realHeight / 2 ) / TERRAIN.TILE_SIZE ) - 1;

	}

	xTileToWorld( x ) {

		return ( x + 0.5 ) * TERRAIN.TILE_SIZE - ( this.realWidth / 2 );

	}
	yTileToWorld( y ) {

		return ( - y - 1 + this.realHeight / TERRAIN.TILE_SIZE + 0.5 ) * TERRAIN.TILE_SIZE - this.realHeight / 2;
	...
	updateTilemap() {
		...
		for ( let i = 0; i < app.dirtyEntities.length; i ++ )
			if ( app.dirtyEntities[ i ].ignore || ! ( app.dirtyEntities[ i ] instanceof Destructible ) || ( app.dirtyEntities[ i ] instanceof Unit && ! app.dirtyEntities[ i ].structure && app.dirtyEntities[ i ].owner === app.localPlayer ) )
